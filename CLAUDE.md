...
- **Clean Architecture**, adapted from the web repo's backend layering,
  ...
  - `data/` — DTOs (`data/remote/dto/`), repository *implementations*,
    the Retrofit service interface (`core/network/`), the Room cache
    (`data/local/` — entities, DAOs, `LocalTodoDataSource`), and the
    offline sync engine (`data/sync/SyncManager.kt`). See "Offline
    support" below for how the last two actually work.
...
## Offline support — local-first cache + sync queue

Diego's spec (2026-08-15): the app should be fully usable with no
connection at all — view and edit every list/item — not just tolerant of
a flaky one. The one exception is registration, which needs the network
regardless (same for login with fresh credentials and account deletion —
see below). `data/local/` (Room) and `data/sync/SyncManager.kt` are the
whole mechanism; `TodoListRepository`'s doc comment has the short version,
this is the long one.

- **Local-first, not cache-then-network.** `TodoListRepositoryImpl`'s
  reads (`observeTodoLists`/`observeTodoList`) are Room `Flow`s — the
  on-device cache *is* what the UI sees, always, online or not. Writes
  apply to that cache immediately (optimistic) and enqueue a
  `PendingOperationEntity`; `SyncManager` pushes the queue once there's a
  connection (`ConnectivityObserver`, a `ConnectivityManager.NetworkCallback`
  wrapped as `Flow<Boolean>`), then does one full pull that merges server
  truth back in.
- **Stable local keys, not reused server ids — this was a real bug caught
  before it shipped, not a hypothetical.** The first version of this
  design gave a temporary negative id to an offline-created row and
  rewrote it in place (`UPDATE ... SET id = ...`) once the real server id
  came back. That breaks any `Flow` already observing that row by its old
  id — e.g. `TodoListDetailScreen` open on a list at the exact moment it
  finishes syncing would suddenly see "not found". Fixed by splitting
  identity in two: `TodoListEntity.localKey`/`TodoItemEntity.localKey`
  (Room-autogenerated, **never changes**, and is what the domain layer's
  `TodoList.id`/`TodoItem.id` actually are) vs. `serverId` (nullable,
  sync-internal, never surfaces past `data/local/`). Nothing above
  `data/local` ever needs to know which server id a list has, so nothing
  above it can be broken by that id changing.
- **The queue doesn't remap ids either, for the same reason — it resolves
  them lazily.** `PendingOperationEntity` stores `listLocalKey`/
  `itemLocalKey` (stable) plus one `payload` string. `SyncManager` looks
  up the *current* server id via `LocalTodoDataSource` right before each
  API call (`requireServerListId`/`requireServerIds`) — this works because
  the queue is strictly FIFO (`id ASC`, one op at a time, re-querying the
  DB each iteration): a `CREATE_LIST` always finishes (and sets that
  list's `serverId`) before any later op referencing the same
  `listLocalKey` gets read. The one exception is `DELETE_LIST`/
  `DELETE_ITEM`: the local row is gone the moment the delete is
  optimistically applied, so there's nothing left to look up — those two
  op types capture the server id directly into `payload` *at enqueue
  time* instead (and are only enqueued at all if the row had one — a
  list/item created offline and deleted again before ever syncing just
  gets its queued ops pruned, no network call at all).
- **"Remote wins" means the pull after a push is authoritative, not that
  conflicts are detected and resolved.** `SyncManager.pushQueue` drops
  (doesn't retry) any op the server rejects outright — an `HttpException`
  (404 because something was deleted elsewhere, 400 against the server's
  current state) — since retrying the identical request can't succeed and
  the merge pull right after is what actually settles the disagreement,
  not the abandoned local intent. An `IOException` (a real network drop
  mid-push) stops the loop instead, leaving that op and everything queued
  behind it for the next connectivity event — order is never
  reshuffled or partially skipped.
- **The merge pull preserves local keys for rows that already existed.**
  `LocalTodoDataSource.mergeServerState` matches incoming server rows
  against the cache by `serverId` (`TodoListDao.getSyncedKeyMap` /
  `TodoItemDao.getSyncedKeyMapForList`) and *updates* the matching local
  row rather than deleting and reinserting everything — a blind
  clear-and-reinsert would reassign every list's `localKey` on every
  background sync, which is the exact bug described above, just
  triggered by an ordinary pull instead of a create. A synced row no
  longer present in the pull (deleted server-side, by this device or
  another client) is the one case that does get deleted locally.
- **Known, accepted trade-off: mutations don't throw synchronously
  anymore.** Before this, `TodoListRepository`'s methods called the API
  directly and a `ViewModel` could catch `ApiException` to show an inline
  error (a duplicate name, a validation failure). A local write always
  succeeds now — that kind of rejection only shows up later, async, via
  `SyncManager.status.lastError` and the small `SyncStatusLine` composable
  (shared by both list screens) rather than at the moment of the action
  that caused it. Deliberate cost of going local-first, not an oversight.
- **`dueDateState` is a local approximation until the next sync.** The
  backend computes Overdue/Today/Upcoming server-side, deliberately never
  client-side (see `domain/model/TodoItem.kt`'s doc comment — timezone
  correctness). `TodoListRepositoryImpl.computeDueDateStateLocally`
  breaks that rule on purpose, but only for optimistic offline display
  using the device's own clock — corrected for real the next time a sync
  pull completes.
- **What's deliberately still online-only**: registering a new account,
  logging in with fresh credentials (an already-issued token still works
  offline — `AppEntryViewModel` only ever reads it from `TokenStore`,
  never calls the network to check), and account deletion. The last one
  is a deliberate scope decision, not an oversight: it's irreversible and
  security-sensitive in a way none of the todo-list domain is, so it
  stays a synchronous, immediately-verifiable server action rather than
  something that can sit queued.
- **Logout wipes the cache** (`AuthRepositoryImpl.logout` →
  `LocalTodoDataSource.clearAll`) — otherwise a different account signing
  in on the same device would see the previous account's lists, and any
  still-queued offline changes would try to push against the wrong
  account.
- Tests: `app/src/test/.../data/sync/SyncManagerTest.kt` — the risky part
  (FIFO ordering, lazy server-id resolution, the two different failure
...
- ~~No offline story~~ **Done** — local-first cache (Room) + a pending-
  mutation sync queue, remote wins on conflict. Full design, including a
  real id-stability bug caught and fixed before it shipped, in the
  dedicated "Offline support" section above.
...
- **App icon is legible now, not designed.** Fixed a real problem
  ...
  The separate 512×512 PNG the Play Store *listing* needs (as opposed to
  the APK's own adaptive icon) is done too —
  `play-store-assets/icon-512.png`, produced by rendering the same mark
  to an in-memory canvas and relaying it to disk over a tiny local HTTP
  server rather than transcribing base64 by hand (see `docs/daily-notes/
  2026-08-15.md` for that technique if it's ever needed again).
